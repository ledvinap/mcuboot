...
use std::{
    collections::{BTreeMap, HashSet}, fs, io::{Cursor, Write}, mem, rc::Rc, slice
};
use aes::{
    Aes128,
    Aes128Ctr,
...
/// ExternalImages represents a simulation where image bytes are provided
/// externally (for example, generated by imgtool).
...
impl ImagesBuilder {
    ...
    /// Construct ExternalImages using prebuilt image binaries.
    /// This currently supports single-image configurations only.
    pub fn make_external_images(self, image0: &str, image1: Option<&str>) -> Result<ExternalImages, String> {
        if self.num_images() != 1 {
            return Err("external images only supported for single-image builds".to_string());
        }

        let mut flash = self.flash;
        let slots = self.slots;

        write_external_image(&mut flash, &slots[0][0], image0)?;
        if let Some(path) = image1 {
            write_external_image(&mut flash, &slots[0][1], path)?;
        }
...
impl ExternalImages {
    /// Run a single boot using externally-provided images.
    /// If `upgrade` is true, marks the secondary slot for upgrade.
    /// If `confirm` is true, marks the primary as confirmed and boots again.
    pub fn run_boot_once(&mut self, upgrade: bool, confirm: bool) -> bool {
        let mut ok = true;
        if upgrade {
            mark_upgrade(&mut self.flash, &self.slots[0][1]);
        }

        if !c::boot_go(&mut self.flash, &self.areadesc, None, None, false).success() {
            return false;
        }

        if upgrade {
            if !verify_trailer(&self.flash, &self.slots[0][0], BOOT_MAGIC_GOOD,
                               BOOT_FLAG_UNSET, BOOT_FLAG_SET) {
                ok = false;
            }
            if !verify_trailer(&self.flash, &self.slots[0][1], BOOT_MAGIC_UNSET,
        ...
        if confirm {
            mark_permanent_upgrade(&mut self.flash, &self.slots[0][0]);
            if !c::boot_go(&mut self.flash, &self.areadesc, None, None, false).success() {
                return false;
            }
            if !verify_trailer(&self.flash, &self.slots[0][0], BOOT_MAGIC_GOOD,
...
fn write_external_image(flash: &mut SimMultiFlash, slot: &SlotInfo, path: &str) -> Result<(), String> {
    let mut buf = fs::read(path).map_err(|e| format!("failed to read {}: {}", path, e))?;
    let dev = flash.get_mut(&slot.dev_id).ok_or_else(|| "missing flash device".to_string())?;

    let mut offset = slot.base_off;
    if Caps::SwapUsingOffset.present() && slot.index == 1 {
        let sector_size = dev.sector_iter().next().unwrap().size as usize;
        offset += sector_size;
    }

    let used = offset - slot.base_off;
    if used >= slot.len {
        return Err("slot offset exceeds slot length".to_string());
    }
    let max_len = slot.len - used;

    let align = dev.align();
    if buf.len() > max_len {
        return Err(format!("image {} is too large for slot ({} > {})", path, buf.len(), max_len));
    }
    if buf.len() % align != 0 {
        let pad = align - (buf.len() % align);
        if buf.len() + pad > max_len {
            return Err(format!("image {} does not fit after alignment padding", path));
        }
        buf.extend(std::iter::repeat(dev.erased_val()).take(pad));
    }
